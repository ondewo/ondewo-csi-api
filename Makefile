...
ONDEWO_CSI_API_VERSION=5.4.0
ONDEWO_NLU_API_GIT_BRANCH=tags/6.11.0
ONDEWO_S2T_API_GIT_BRANCH=tags/7.3.0
...
githubio_logic_pre:
	...
	@mkdir "${DOCS_DIR}"
	@cp docs/* ${DOCS_DIR}
	@sed -i "s/h1>Protocol Documentation/h1>${REPO_NAME_UPPER} ${ONDEWO_CSI_API_VERSION} Documentation/" ${DOCS_DIR}/index.html

githubio_logic: | githubio_logic_pre
	$(eval REPO_NAME:= $(shell echo ${GH_REPO} | cut -d "-" -f 2 ))
	$(eval REPO_NAME_UPPER:= $(shell echo ${GH_REPO} | cut -d "-" -f 2 | sed -e 's/\(.*\)/\U\1/'))
	@git branch | grep "*" | grep -q "master" || (echo "Not on master branch"  & rm -rf ondewo.github.io && exit 1)
	@! cat ondewo.github.io/data.js | sed -n "/name\: '${REPO_NAME_UPPER}'/,/end\: ''/p" | grep -q "number: '${ONDEWO_CSI_API_VERSION}'" || (echo "Already Released" && exit 1)
	...
	$(eval TEMP_TEXT:= $(shell cat ondewo.github.io/script_object.txt | sed -e "s/VERSION/${ONDEWO_CSI_API_VERSION}/g" -e "s/TECHNOLOGY/${REPO_NAME}/g"))
	...
	@cp docs/* ${DOCS_DIR}
	@sed -i "s/h1>Protocol Documentation/h1>${REPO_NAME_UPPER} ${ONDEWO_CSI_API_VERSION} Documentation/" ${DOCS_DIR}/index.html
	$(eval HEADER_LINE:= $(shell cat ${DOCS_DIR}/index.html | grep -n "${REPO_NAME_UPPER} ${ONDEWO_CSI_API_VERSION} Documentation" | grep -o -E '[0-9]+' | head -1 | sed -e 's/^0\+//'))
...
# Change Version Number and RELEASE NOTES
	...
	cd ${REPO_DIR} && sed -i -e 's/ONDEWO_CSI_VERSION.*=.*/ONDEWO_CSI_VERSION = ${ONDEWO_CSI_API_VERSION}/' Makefile
	cd ${REPO_DIR} && sed -i -e 's/ONDEWO_PROTO_COMPILER_GIT_BRANCH.*=.*/ONDEWO_PROTO_COMPILER_GIT_BRANCH=tags\/${PROTO_COMPILER}/' Makefile
	cd ${REPO_DIR} && sed -i -e 's/CSI_API_GIT_BRANCH.*=.*/CSI_API_GIT_BRANCH=tags\/${ONDEWO_CSI_API_VERSION}/' Makefile && head -30 Makefile
