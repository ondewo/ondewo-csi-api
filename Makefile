...
# `\<newline><tab>` to a further space, so the block tripped MD009/MD007/MD022/MD012/MD032 in EVERY client and the first
# pre-commit run of every release `Failed - files were modified by this hook`. It self-healed on the re-run,
# but only after markdownlint had rewritten the entry the release had just written. Keep this byte-identical
# to what markdownlint normalises to: no trailing spaces, a blank line around the heading and the list.
# The value carries no quotes of its own either - the recipe below reads it from the environment.
